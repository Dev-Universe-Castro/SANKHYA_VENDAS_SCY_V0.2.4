    ...
    try {
        ...
        if (idSistema && empresa) {
            ...
            const resultado = type === 'partial'
                ? await sincronizarMarcasParcial(parseInt(idSistema as string), empresa as string)
                : await sincronizarMarcasTotal(parseInt(idSistema as string), empresa as string);
