  ...
  try {
    ...
    if (idSistema && empresa) {
      ...
      const resultado = type === 'partial'
        ? await sincronizarTiposOperacaoParcial(parseInt(idSistema as string), empresa as string)
        : await sincronizarTiposOperacaoTotal(parseInt(idSistema as string), empresa as string);
