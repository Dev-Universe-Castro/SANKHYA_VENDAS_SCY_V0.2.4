...
import { RefreshCw, Clock, Database, Tag, MapPin } from "lucide-react"
import {
    Table,
    TableBody,
    TableCell,
...
export default function SincronizacaoBairrosPage() {
    ...
    const loadData = async () => {
        try {
            ...
            const statsRes = await fetch('/api/sync/bairros')
            if (statsRes.ok) {
            ...
            fetch('/api/sync/bairros?list=true')
                .then(res => res.json())
                .then(data => {
                    if (Array.isArray(data)) {
                        setBairros(data);
                    } else {
                        console.error('Dados de bairros inválidos:', data);
                        setBairros([]);
                    }
                })
                .catch(console.error)
    ...
    const sincronizarTodas = async () => {
        try {
            ...
            toast({
                ...
                description: "Sincronizando bairros de todas as empresas..."
            })

            const response = await fetch('/api/sync/bairros', {
                method: 'POST',
    ...
    const sincronizarEmpresa = async (idSistema: number, empresa: string, type: 'total' | 'partial' = 'total') => {
        try {
            ...
            toast({
                ...
                description: `Sincronizando bairros de ${empresa}...`
            })
        ...
        <DashboardLayout>
            <div className="space-y-6 p-6">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                        <h1 className="text-3xl font-bold">Sincronização de Bairros</h1>
                        <p className="text-muted-foreground mt-2">
                            Gerencie a sincronização de Bairros entre Sankhya e o Oracle.
                        </p>
                    </div>
                    <Button
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>
                            Status de sincronização dos bairros por contrato.
                        </CardDescription>
                    </CardHeader>
                    <CardContent>
                        {contratos.length === 0 ? (
                                ...
                                <Table>
                                    <TableHeader>
                                        <TableRow>
                                            ...
                                            <TableHead className="text-center">Total Bairros Sync</TableHead>
                ...
                <Card>
                    <CardHeader>
                        <CardTitle>Bairros Sincronizados (Detalhes)</CardTitle>
                        <CardDescription>
                            Lista de bairros sincronizados do Sankhya.
                        </CardDescription>
                    </CardHeader>
                    <CardContent>
                        <div className="rounded-md border overflow-x-auto">
                            <Table>
                                <TableHeader>
                                    <TableRow>
                                        <TableHead>Cód. Bairro</TableHead>
                                    ...
                                    {bairros.length === 0 ? (
                                        <TableRow>
                                            <TableCell colSpan={6} className="text-center py-8 text-muted-foreground">
                                                Nenhum bairro sincronizado encontrado.
                                            </TableCell>
                                        </TableRow>
                                    ) : (
                                        bairros.map((bairro) => (
                                            <TableRow key={`${bairro.ID_SISTEMA}-${bairro.CODBAI}`}>
                                                <TableCell>{bairro.CODBAI}</TableCell>
                                                <TableCell>{bairro.NOMEBAI}</TableCell>
                                                <TableCell>{bairro.CODREG}</TableCell>
                                                <TableCell className="text-xs text-muted-foreground">
                                                    {bairro.NOME_CONTRATO} (ID: {bairro.ID_SISTEMA})
                                                </TableCell>
                                                <TableCell>
                                                    {bairro.SANKHYA_ATUAL === 'S' ? (
                                                ...
                                                <TableCell className="text-xs text-muted-foreground">
                                                    {formatarData(bairro.DT_ULT_CARGA)}
                                                </TableCell>
                                            </TableRow>
                                        ))
                                    )}
