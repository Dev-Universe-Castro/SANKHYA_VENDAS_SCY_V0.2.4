...
export default function SincronizacaoExcecaoPrecoPage() {
  ...
  const loadData = async () => {
    try {
      ...
      const statsRes = await fetch('/api/sync/excecao-preco')
      if (statsRes.ok) {
      ...
      fetch('/api/sync/excecao-preco?list=true')
        .then(res => res.json())
  ...
  const sincronizarTodas = async () => {
    try {
      ...
      toast({
        ...
        description: "Sincronizando exceções de preço de todas as empresas..."
      })

      const response = await fetch('/api/sync/excecao-preco', {
        method: 'POST'
      })
  ...
  const sincronizarEmpresa = async (idSistema: number, empresa: string, type: 'total' | 'partial' = 'total') => {
    try {
      ...
      toast({
        ...
        description: `Sincronizando exceções de preço de ${empresa}...`
      })
    ...
    <DashboardLayout>
      <div className="space-y-6 p-6">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h1 className="text-3xl font-bold">Sincronização de Exceções de Preço</h1>
            <p className="text-muted-foreground mt-2">
              Gerencie a sincronização de exceções de preço entre Sankhya e o Oracle.
            </p>
          </div>
          <Button
        ...
        <div className="grid gap-6 md:grid-cols-3">
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Total de Exceções</CardTitle>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">
                Exceções de preço ativas sincronizadas
              </p>
            </CardContent>
          </Card>
        </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Exceções de Preço Sincronizadas (Detalhes)</CardTitle>
            <CardDescription>
              Lista de exceções de preço sincronizadas do Sankhya (últimos 500 records).
            </CardDescription>
          </CardHeader>
          <CardContent>
                  ...
                  {excecoes.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={7} className="text-center py-8 text-muted-foreground">
                        Nenhuma exceção de preço sincronizada encontrada.
                      </TableCell>
                    </TableRow>
                  ) : (
                    excecoes.map((item) => (
                      <TableRow key={`${item.ID_SISTEMA}-${item.CODPROD}-${item.NUTAB}-${item.CODLOCAL}`}>
                        <TableCell>{item.CODPROD}</TableCell>
                        <TableCell>{item.NUTAB}</TableCell>
                        <TableCell>{item.CODLOCAL}</TableCell>
                        <TableCell>{formatarMoeda(item.VLRVENDA)}</TableCell>
