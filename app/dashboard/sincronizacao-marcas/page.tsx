...
import { RefreshCw, Clock, Database, Tag } from "lucide-react"
import {
    Table,
    TableBody,
    TableCell,
...
export default function SincronizacaoMarcasPage() {
    ...
    const loadData = async () => {
        try {
            ...
            const statsRes = await fetch('/api/sync/marcas')
            if (statsRes.ok) {
            ...
            fetch('/api/sync/marcas?list=true')
                .then(res => res.json())
                .then(data => {
                    if (Array.isArray(data)) {
                        setMarcas(data);
                    } else {
                        console.error('Dados de marcas inválidos:', data);
                        setMarcas([]);
                    }
                })
                .catch(console.error)
    ...
    const sincronizarTodas = async () => {
        try {
            ...
            toast({
                ...
                description: "Sincronizando marcas de todas as empresas..."
            })

            const response = await fetch('/api/sync/marcas', {
                method: 'POST'
            })
    ...
    const sincronizarEmpresa = async (idSistema: number, empresa: string, type: 'total' | 'partial' = 'total') => {
        try {
            ...
            toast({
                title: `Sincronização ${type === 'total' ? 'Total' : 'Parcial'} iniciada`,
                description: `Sincronizando marcas de ${empresa}...`
            })
        ...
        <DashboardLayout>
            <div className="space-y-6 p-6">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                        <h1 className="text-3xl font-bold">Sincronização de Marcas</h1>
                        <p className="text-muted-foreground mt-2">
                            Gerencie a sincronização de marcas de produtos entre Sankhya e o Oracle.
                        </p>
                    </div>
                    <Button
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>
                            Status de sincronização das marcas por contrato.
                        </CardDescription>
                    </CardHeader>
                    <CardContent>
                        {contratos.length === 0 ? (
                ...
                <Card>
                    <CardHeader>
                        <CardTitle>Marcas Sincronizadas (Detalhes)</CardTitle>
                        <CardDescription>
                            Lista de marcas sincronizadas do Sankhya (últimos 500).
                        </CardDescription>
                    </CardHeader>
                    <CardContent>
                                    ...
                                    {marcas.length === 0 ? (
                                        <TableRow>
                                            <TableCell colSpan={5} className="text-center py-8 text-muted-foreground">
                                                Nenhuma marca sincronizada encontrada.
                                            </TableCell>
                                        </TableRow>
                                    ) : (
                                        marcas.map((marca) => (
                                            <TableRow key={`${marca.ID_SISTEMA}-${marca.CODIGO}`}>
                                                <TableCell>{marca.CODIGO}</TableCell>
                                                <TableCell>{marca.DESCRICAO}</TableCell>
                                                <TableCell className="text-xs text-muted-foreground">
                                                    {marca.NOME_CONTRATO} (ID: {marca.ID_SISTEMA})
                                                </TableCell>
                                                <TableCell>
                                                    {marca.SANKHYA_ATUAL === 'S' ? (
                                                ...
                                                <TableCell className="text-xs text-muted-foreground">
                                                    {formatarData(marca.DT_ULT_CARGA)}
                                                </TableCell>
                                            </TableRow>
                                        ))
                                    )}
