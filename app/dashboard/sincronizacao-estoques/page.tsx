...
import { RefreshCw, Clock, Database, Tag, Package, ListOrdered } from "lucide-react"
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
export default function SincronizacaoEstoquesPage() {
  ...
  const loadData = async () => {
    try {
      ...
      const statsRes = await fetch('/api/sync/estoques')
      if (statsRes.ok) {
      ...
      fetch('/api/sync/estoques?list=true')
        .then(res => res.json())
  ...
  const sincronizarTodas = async () => {
    try {
      ...
      toast({
        ...
        description: "Sincronizando estoques de todas as empresas..."
      })

      const response = await fetch('/api/sync/estoques', {
        method: 'POST',
        headers: {
  ...
  const sincronizarEmpresa = async (idSistema: number, empresa: string, type: 'total' | 'partial' = 'total', startPage?: number) => {
    try {
      ...
      toast({
        ...
        description: `Sincronizando estoques de ${empresa}${startPage !== undefined ? ` a partir da página ${startPage}` : ''}...`
      })
    ...
    <DashboardLayout>
      <div className="space-y-6 p-6">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h1 className="text-3xl font-bold">Sincronização de Estoques</h1>
            <p className="text-muted-foreground mt-2">
              Gerencie a sincronização de estoques entre Sankhya e o Oracle.
            </p>
          </div>
          <Button
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {formatarNumero(Array.from(estatisticas.values()).reduce((acc, curr) => acc + curr.ESTOQUE_TOTAL, 0))}
              </div>
              <p className="text-xs text-muted-foreground">
                Soma total de todos os estoques sincronizados
              </p>
            </CardContent>
          </Card>
        </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Status de sincronização por contrato.
            </CardDescription>
          </CardHeader>
          <CardContent>
            {contratos.length === 0 ? (
                            ...
                            {stats ? (
                              <div className="flex items-center justify-center gap-1">
                                ...
                                <span className="font-semibold text-blue-600">
                                  {formatarNumero(stats.ESTOQUE_TOTAL)}
                                </span>
                              </div>
                            ) : (
                              <span className="text-muted-foreground">-</span>
        ...
        <Dialog open={isPageDialogOpen} onOpenChange={setIsPageDialogOpen}>
          <DialogContent className="sm:max-w-[425px]">
            <DialogHeader>
              <DialogTitle>Sincronização por Página (Estoques)</DialogTitle>
              <DialogDescription>
                Escolha a página inicial para a sincronização de estoques de <strong>{selectedContrato?.EMPRESA}</strong>.
              </DialogDescription>
            </DialogHeader>
            <div className="grid gap-4 py-4">
              ...
              <p className="text-xs text-muted-foreground bg-yellow-50 p-2 rounded border border-yellow-200">
                Atenção: A sincronização por página ignora o reset inicial (Soft Delete) e apenas atualiza/insere estoques encontrados a partir da página informada.
              </p>
            </div>
            <DialogFooter>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Estoques Sincronizados (Detalhes)</CardTitle>
            <CardDescription>
              Lista de estoques sincronizados do Sankhya (últimos 500).
            </CardDescription>
          </CardHeader>
          <CardContent>
                  ...
                  {estoques.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={7} className="text-center py-8 text-muted-foreground">
                        Nenhum estoque sincronizado encontrado.
                      </TableCell>
                    </TableRow>
                  ) : (
                    estoques.map((item) => (
                      <TableRow key={`${item.ID_SISTEMA}-${item.CODPROD}-${item.CODLOCAL}`}>
                        <TableCell>{item.CODPROD}</TableCell>
                        <TableCell>{item.CODLOCAL}</TableCell>
                        <TableCell>{formatarNumero(item.ESTOQUE)}</TableCell>
                        <TableCell>{item.CONTROLE}</TableCell>
                        <TableCell className="text-xs text-muted-foreground">
                          {item.NOME_CONTRATO} (ID: {item.ID_SISTEMA})
                        </TableCell>
                        <TableCell>
                          {item.SANKHYA_ATUAL === 'S' ? (
                        ...
                        <TableCell className="text-xs text-muted-foreground">
                          {formatarData(item.DT_ULT_CARGA)}
                        </TableCell>
                      </TableRow>
                    ))
                  )}
