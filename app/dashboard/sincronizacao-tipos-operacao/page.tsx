...
import { RefreshCw, Clock, Database, Tag, Layers } from "lucide-react"
import {
  Table,
  TableBody,
  TableCell,
...
export default function SincronizacaoTiposOperacaoPage() {
  ...
  const loadData = async () => {
    try {
      ...
      const statsRes = await fetch('/api/sync/tipos-operacao')
      if (statsRes.ok) {
      ...
      fetch('/api/sync/tipos-operacao?list=true')
        .then(res => res.json())
        .then(data => setTiposOperacao(data))
        .catch(console.error)
  ...
  const sincronizarTodas = async () => {
    try {
      ...
      toast({
        ...
        description: "Sincronizando tipos de operação de todas as empresas..."
      })

      const response = await fetch('/api/sync/tipos-operacao', {
        method: 'POST'
      })
  ...
  const sincronizarEmpresa = async (idSistema: number, empresa: string, type: 'total' | 'partial' = 'total') => {
    try {
      ...
      toast({
        ...
        description: `Sincronizando tipos de operação de ${empresa}...`
      })
    ...
    <DashboardLayout>
      <div className="space-y-6 p-6">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h1 className="text-3xl font-bold">Sincronização de Tipos de Operação</h1>
            <p className="text-muted-foreground mt-2">
              Gerencie a sincronização de tipos de operação entre Sankhya e o Oracle.
            </p>
          </div>
          <Button
        ...
        <div className="grid gap-6 md:grid-cols-3">
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Total de Tipos de Operação</CardTitle>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">
                Tipos de operação ativos sincronizados
              </p>
            </CardContent>
          </Card>
        </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Tipos de Operação Sincronizados (Detalhes)</CardTitle>
            <CardDescription>
              Lista de tipos de operação sincronizados do Sankhya (últimos 500).
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="rounded-md border overflow-x-auto">
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead>Cód. Tipo Oper.</TableHead>
                  ...
                  {tiposOperacao.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={5} className="text-center py-8 text-muted-foreground">
                        Nenhum tipo de operação sincronizado encontrado.
                      </TableCell>
                    </TableRow>
                  ) : (
                    tiposOperacao.map((item) => (
                      <TableRow key={`${item.ID_SISTEMA}-${item.CODTIPOPER}`}>
                        <TableCell>{item.CODTIPOPER}</TableCell>
                        <TableCell>{item.DESCROPER}</TableCell>
