...
import { RefreshCw, Clock, Database, Tag, DollarSign } from "lucide-react"
import {
  Table,
  TableBody,
  TableCell,
...
export default function SincronizacaoTabelaPrecosPage() {
  ...
  const loadData = async () => {
    try {
      ...
      const statsRes = await fetch('/api/sync/tabela-precos')
      if (statsRes.ok) {
      ...
      fetch('/api/sync/tabela-precos?list=true')
        .then(res => res.json())
        .then(data => setTabelaPrecos(data))
        .catch(console.error)
  ...
  const sincronizarTodas = async () => {
    try {
      ...
      toast({
        ...
        description: "Sincronizando tabelas de preços de todas as empresas..."
      })

      const response = await fetch('/api/sync/tabela-precos', {
        method: 'POST'
      })
  ...
  const sincronizarEmpresa = async (idSistema: number, empresa: string, type: 'total' | 'partial' = 'total') => {
    try {
      ...
      toast({
        ...
        description: `Sincronizando tabelas de preços de ${empresa}...`
      })
    ...
    <DashboardLayout>
      <div className="space-y-6 p-6">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h1 className="text-3xl font-bold">Sincronização de Tabelas de Preços</h1>
            <p className="text-muted-foreground mt-2">
              Gerencie a sincronização de tabelas de preços entre Sankhya e o Oracle.
            </p>
          </div>
          <Button
        ...
        <div className="grid gap-6 md:grid-cols-3">
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Total de Tabelas de Preços</CardTitle>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">
                Tabelas de preços ativas sincronizadas
              </p>
            </CardContent>
          </Card>
        </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Tabelas de Preços Sincronizadas (Detalhes)</CardTitle>
            <CardDescription>
              Lista de tabelas de preços sincronizadas do Sankhya (últimos 500 records).
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="rounded-md border overflow-x-auto">
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead>Núm. Tab.</TableHead>
                    <TableHead>Data Vigor</TableHead>
                  ...
                  {tabelaPrecos.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={6} className="text-center py-8 text-muted-foreground">
                        Nenhuma tabela de preços sincronizada encontrada.
                      </TableCell>
                    </TableRow>
                  ) : (
                    tabelaPrecos.map((item) => (
                      <TableRow key={`${item.ID_SISTEMA}-${item.NUTAB}`}>
                        <TableCell>{item.NUTAB}</TableCell>
                        <TableCell>{formatarData(item.DTVIGOR)}</TableCell>
