...
import { RefreshCw, Clock, Database, Tag, Map as MapIcon } from "lucide-react"
import {
    Table,
    TableBody,
    TableCell,
...
export default function SincronizacaoRegioesPage() {
    ...
    const loadData = async () => {
        try {
            ...
            const statsRes = await fetch('/api/sync/regioes')
            if (statsRes.ok) {
            ...
            fetch('/api/sync/regioes?list=true')
                .then(res => res.json())
                .then(data => {
                    if (Array.isArray(data)) {
                        setRegioes(data);
                    } else {
                        console.error('Dados de regiões inválidos:', data);
                        setRegioes([]);
                    }
                })
                .catch(console.error)
    ...
    const sincronizarTodas = async () => {
        try {
            ...
            toast({
                ...
                description: "Sincronizando regiões de todas as empresas..."
            })

            const response = await fetch('/api/sync/regioes', {
                method: 'POST',
    ...
    const sincronizarEmpresa = async (idSistema: number, empresa: string, type: 'total' | 'partial' = 'total') => {
        try {
            ...
            toast({
                ...
                description: `Sincronizando regiões de ${empresa}...`
            })
        ...
        <DashboardLayout>
            <div className="space-y-6 p-6">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                        <h1 className="text-3xl font-bold">Sincronização de Regiões</h1>
                        <p className="text-muted-foreground mt-2">
                            Gerencie a sincronização de Regiões entre Sankhya e o Oracle.
                        </p>
                    </div>
                    <Button
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>
                            Status de sincronização das regiões por contrato.
                        </CardDescription>
                    </CardHeader>
                    <CardContent>
                        {contratos.length === 0 ? (
                                ...
                                <Table>
                                    <TableHeader>
                                        <TableRow>
                                            ...
                                            <TableHead className="text-center">Total Regiões Sync</TableHead>
                ...
                <Card>
                    <CardHeader>
                        <CardTitle>Regiões Sincronizadas (Detalhes)</CardTitle>
                        <CardDescription>
                            Lista de regiões sincronizadas do Sankhya.
                        </CardDescription>
                    </CardHeader>
                    <CardContent>
                                    ...
                                    {regioes.length === 0 ? (
                                        <TableRow>
                                            <TableCell colSpan={6} className="text-center py-8 text-muted-foreground">
                                                Nenhuma região sincronizada encontrada.
                                            </TableCell>
                                        </TableRow>
                                    ) : (
                                        regioes.map((reg) => (
                                            <TableRow key={`${reg.ID_SISTEMA}-${reg.CODREG}`}>
                                                <TableCell>{reg.CODREG}</TableCell>
                                                <TableCell>{reg.NOMEREG}</TableCell>
                                                <TableCell>{reg.ATIVA === 'S' ? 'Sim' : 'Não'}</TableCell>
                                                <TableCell className="text-xs text-muted-foreground">
                                                    {reg.NOME_CONTRATO} (ID: {reg.ID_SISTEMA})
                                                </TableCell>
                                                <TableCell>
                                                    {reg.SANKHYA_ATUAL === 'S' ? (
