...
import { RefreshCw, Clock, Database, Users, Tag } from "lucide-react"
import {
  Table,
  TableBody,
  TableCell,
...
export default function SincronizacaoVendedoresPage() {
  ...
  const loadData = async () => {
    try {
      ...
      const statsRes = await fetch('/api/sync/vendedores')
      if (statsRes.ok) {
      ...
      fetch('/api/sync/vendedores?list=true')
        .then(res => res.json())
        .then(data => {
  ...
  const sincronizarTodas = async () => {
    try {
      ...
      toast({
        ...
        description: "Sincronizando vendedores de todas as empresas..."
      })

      const response = await fetch('/api/sync/vendedores', {
        method: 'POST'
      })
  ...
  const sincronizarEmpresa = async (idSistema: number, empresa: string, type: 'total' | 'partial' = 'total') => {
    try {
      ...
      toast({
        ...
        description: `Sincronizando vendedores de ${empresa}...`
      })
    ...
    <DashboardLayout>
      <div className="space-y-6 p-6">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h1 className="text-3xl font-bold">Sincronização de Vendedores e Gerentes</h1>
            <p className="text-muted-foreground mt-2">
              Gerencie a sincronização de vendedores e gerentes entre Sankhya e o Oracle.
            </p>
          </div>
          <Button
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">
                Vendedores ativos sincronizados
              </p>
            </CardContent>
          </Card>
        </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Vendedores Sincronizados (Detalhes)</CardTitle>
            <CardDescription>
              Lista de vendedores sincronizados do Sankhya (últimos 500 records).
            </CardDescription>
          </CardHeader>
          <CardContent>
                  ...
                  {vendedores.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={7} className="text-center py-8 text-muted-foreground">
                        Nenhum vendedor sincronizado encontrado.
                      </TableCell>
                    </TableRow>
                  ) : (
                    vendedores.map((item) => (
                      <TableRow key={`${item.ID_SISTEMA}-${item.CODVEND}`}>
                        <TableCell>{item.CODVEND}</TableCell>
                        <TableCell>{item.APELIDO}</TableCell>
                        <TableCell>{item.TIPVEND}</TableCell>
                        <TableCell>{item.EMAIL || '-'}</TableCell>
                        <TableCell className="text-xs text-muted-foreground">
                          {item.NOME_CONTRATO} (ID: {item.ID_SISTEMA})
                        </TableCell>
                        <TableCell>
                          {item.ATIVO === 'S' || item.SANKHYA_ATUAL === 'S' ? ( // Adjust logic as needed, assuming ATIVO from Sankhya or SANKHYA_ATUAL
