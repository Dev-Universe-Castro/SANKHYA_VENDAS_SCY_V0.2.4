...
export default function SincronizacaoCidadesPage() {
    ...
    const loadData = async () => {
        try {
            ...
            const statsRes = await fetch('/api/sync/cidades')
            if (statsRes.ok) {
            ...
            fetch('/api/sync/cidades?list=true')
                .then(res => res.json())
                .then(data => {
    ...
    const sincronizarTodas = async () => {
        try {
            ...
            toast({
                ...
                description: "Sincronizando cidades de todas as empresas..."
            })

            const response = await fetch('/api/sync/cidades', {
                method: 'POST',
    ...
    const sincronizarEmpresa = async (idSistema: number, empresa: string, type: 'total' | 'partial' = 'total') => {
        try {
            ...
            toast({
                ...
                description: `Sincronizando cidades de ${empresa}...`
            })
        ...
        <DashboardLayout>
            <div className="space-y-6 p-6">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                        <h1 className="text-3xl font-bold">Sincronização de Cidades</h1>
                        <p className="text-muted-foreground mt-2">
                            Gerencie a sincronização de Cidades entre Sankhya e o Oracle.
                        </p>
                    </div>
                    <Button
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>
                            Status de sincronização das cidades por contrato.
                        </CardDescription>
                    </CardHeader>
                    <CardContent>
                        {contratos.length === 0 ? (
                                ...
                                <Table>
                                    <TableHeader>
                                        <TableRow>
                                            ...
                                            <TableHead className="text-center">Total Cidades Sync</TableHead>
                ...
                <Card>
                    <CardHeader>
                        <CardTitle>Cidades Sincronizadas (Detalhes)</CardTitle>
                        <CardDescription>
                            Lista de cidades sincronizadas do Sankhya.
                        </CardDescription>
                    </CardHeader>
                    <CardContent>
                        <div className="rounded-md border overflow-x-auto">
                            <Table>
                                <TableHeader>
                                    <TableRow>
                                        <TableHead>Cód. Cidade</TableHead>
                                    ...
                                    {cidades.length === 0 ? (
                                        <TableRow>
                                            <TableCell colSpan={7} className="text-center py-8 text-muted-foreground">
                                                Nenhuma cidade sincronizada encontrada.
                                            </TableCell>
                                        </TableRow>
                                    ) : (
                                        cidades.map((cid) => (
                                            <TableRow key={`${cid.ID_SISTEMA}-${cid.CODCID}`}>
                                                <TableCell>{cid.CODCID}</TableCell>
                                                <TableCell>{cid.NOMECID}</TableCell>
                                                <TableCell>{cid.UF}</TableCell>
                                                <TableCell>{cid.UFNOMECID}</TableCell>
                                                <TableCell className="text-xs text-muted-foreground">
                                                    {cid.NOME_CONTRATO} (ID: {cid.ID_SISTEMA})
                                                </TableCell>
                                                <TableCell>
                                                    {cid.SANKHYA_ATUAL === 'S' ? (
