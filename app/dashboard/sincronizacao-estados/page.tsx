...
export default function SincronizacaoEstadosPage() {
    ...
    const loadData = async () => {
        try {
            ...
            const statsRes = await fetch('/api/sync/estados')
            if (statsRes.ok) {
            ...
            fetch('/api/sync/estados?list=true')
                .then(res => res.json())
    ...
    const sincronizarTodas = async () => {
        try {
            ...
            toast({
                ...
                description: "Sincronizando estados de todas as empresas..."
            })

            const response = await fetch('/api/sync/estados', {
                method: 'POST',
    ...
    const sincronizarEmpresa = async (idSistema: number, empresa: string, type: 'total' | 'partial' = 'total') => {
        try {
            ...
            toast({
                ...
                description: `Sincronizando estados de ${empresa}...`
            })
        ...
        <DashboardLayout>
            <div className="space-y-6 p-6">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                        <h1 className="text-3xl font-bold">Sincronização de Estados</h1>
                        <p className="text-muted-foreground mt-2">
                            Gerencie a sincronização de Estados (UF) entre Sankhya e o Oracle.
                        </p>
                    </div>
                    <Button
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>
                            Status de sincronização dos estados por contrato.
                        </CardDescription>
                    </CardHeader>
                    <CardContent>
                        {contratos.length === 0 ? (
                                ...
                                <Table>
                                    <TableHeader>
                                        <TableRow>
                                            ...
                                            <TableHead className="text-center">Total Estados Sync</TableHead>
                ...
                <Card>
                    <CardHeader>
                        <CardTitle>Estados Sincronizados (Detalhes)</CardTitle>
                        <CardDescription>
                            Lista de estados sincronizados do Sankhya.
                        </CardDescription>
                    </CardHeader>
                    <CardContent>
                                    ...
                                    {estados.length === 0 ? (
                                        <TableRow>
                                            <TableCell colSpan={6} className="text-center py-8 text-muted-foreground">
                                                Nenhum estado sincronizado encontrado.
                                            </TableCell>
                                        </TableRow>
                                    ) : (
                                        estados.map((est) => (
                                            <TableRow key={`${est.ID_SISTEMA}-${est.CODUF}`}>
                                                <TableCell>{est.CODUF}</TableCell>
                                                <TableCell>{est.UF}</TableCell>
                                                <TableCell>{est.DESCRICAO}</TableCell>
                                                <TableCell className="text-xs text-muted-foreground">
                                                    {est.NOME_CONTRATO} (ID: {est.ID_SISTEMA})
                                                </TableCell>
                                                <TableCell>
                                                    {est.SANKHYA_ATUAL === 'S' ? (
