...
import { RefreshCw, Clock, Database, Tag, Briefcase } from "lucide-react"
import {
  Table,
  TableBody,
  TableCell,
...
export default function SincronizacaoTiposNegociacaoPage() {
  ...
  const [complementosParceiro, setTiposNegociacao] = useState<any[]>([])
  ...
  const loadData = async () => {
    try {
      ...
      const statsRes = await fetch('/api/sync/complemento-parceiro')
      if (statsRes.ok) {
      ...
      fetch('/api/sync/complemento-parceiro?list=true')
        .then(res => res.json())
        .then(data => setTiposNegociacao(data))
        .catch(console.error)
  ...
  const sincronizarTodas = async () => {
    try {
      ...
      toast({
        ...
        description: "Sincronizando complemento de parceiro de todas as empresas..."
      })

      const response = await fetch('/api/sync/complemento-parceiro', {
        method: 'POST'
      })
  ...
  const sincronizarEmpresa = async (idSistema: number, empresa: string, type: 'total' | 'partial' = 'total') => {
    try {
      ...
      toast({
        ...
        description: `Sincronizando complemento de parceiro de ${empresa}...`
      })
    ...
    <DashboardLayout>
      <div className="space-y-6 p-6">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h1 className="text-3xl font-bold">Sincronização de Complemento Parceiro</h1>
            <p className="text-muted-foreground mt-2">
              Gerencie a sincronização de complemento de parceiro entre Sankhya e o Oracle.
            </p>
          </div>
          <Button
        ...
        <div className="grid gap-6 md:grid-cols-3">
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Total de Complemento Parceiro</CardTitle>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">
                Tipos de negociação ativos sincronizados
              </p>
            </CardContent>
          </Card>
        </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Complemento Parceiro Sincronizados (Detalhes)</CardTitle>
            <CardDescription>
              Lista de complemento de parceiro sincronizados do Sankhya (últimos 500).
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="rounded-md border overflow-x-auto">
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead>Cód. Tipo Venda</TableHead>
                ...
                <TableBody>
                  {complementosParceiro.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={5} className="text-center py-8 text-muted-foreground">
                        Nenhum tipo de negociação sincronizado encontrado.
                      </TableCell>
                    </TableRow>
                  ) : (
                    complementosParceiro.map((item) => (
                      <TableRow key={`${item.ID_SISTEMA}-${item.CODTIPVENDA}`}>
                        <TableCell>{item.CODTIPVENDA}</TableCell>
                        <TableCell>{item.DESCRTIPVENDA}</TableCell>
