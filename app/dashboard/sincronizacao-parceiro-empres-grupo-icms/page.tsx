...
import { RefreshCw, Clock, Database, Tag, FileText } from "lucide-react"
import {
    Table,
    TableBody,
    TableCell,
...
export default function SincronizacaoParceiroEmpresGrupoIcmsPage() {
    ...
    const loadData = async () => {
        try {
            ...
            const statsRes = await fetch('/api/sync/parceiro-empres-grupo-icms')
            if (statsRes.ok) {
    ...
    const sincronizarTodas = async () => {
        try {
            ...
            toast({
                ...
                description: "Sincronizando regras de ICMS de todas as empresas..."
            })

            const response = await fetch('/api/sync/parceiro-empres-grupo-icms', {
                method: 'POST',
    ...
    const sincronizarEmpresa = async (idSistema: number, empresa: string) => {
        try {
            ...
            toast({
                title: `Sincronização iniciada`,
                description: `Sincronizando regras de ICMS de ${empresa}...`
            })
        ...
        <DashboardLayout>
            <div className="space-y-6 p-6">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                        <h1 className="text-3xl font-bold">Sincronização de Regras de ICMS Parc.</h1>
                        <p className="text-muted-foreground mt-2">
                            Gerencie a sincronização de regras de ICMS por Parceiro/Empresa entre Sankhya e o Oracle.
                        </p>
                    </div>
                    <Button
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>
                            Status de sincronização das regras por contrato.
                        </CardDescription>
                    </CardHeader>
                    <CardContent>
                        {contratos.length === 0 ? (
                                ...
                                <Table>
                                    <TableHeader>
                                        <TableRow>
                                            ...
                                            <TableHead className="text-center">Total Regras Sync</TableHead>
