...
import { RefreshCw, Clock, Database, UserCircle, Users } from "lucide-react"
import {
  Table,
  TableBody,
  TableCell,
...
export default function SincronizacaoPage() {
  ...
  const loadData = async () => {
    try {
      ...
      fetch('/api/sync/parceiros?list=true')
        .then(res => res.json())
  ...
  const sincronizarEmpresa = async (idSistema: number, empresa: string, type: 'total' | 'partial' = 'total') => {
    try {
      ...
      toast({
        title: "Sincronização iniciada",
        description: `Sincronizando ${empresa} (${type === 'total' ? 'Total' : 'Parcial'})...`
      })
      ...
      <div className="space-y-6 p-6">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            ...
            <p className="text-muted-foreground mt-2">
              Gerencie a sincronização de Parceiros entre Sankhya e o Oracle.
            </p>
          </div>
          <Button
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">
                Parceiros ativos sincronizados
              </p>
            </CardContent>
          </Card>
        </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Status de sincronização dos parceiros por contrato.
            </CardDescription>
          </CardHeader>
          <CardContent>
            {contratos.length === 0 ? (
                          ...
                          <TableCell className="text-right">
                            <div className="flex items-center justify-end gap-2">
                              <Button
                                ...
                                size="sm"
                                variant="outline"
                                title={contrato.SYNC_ATIVO ? "Sincronização automática está ativa" : "Sincronizar apenas registros alterados recentemente"}
                              >
                                {isSyncing ? (
                                  <>
                              ...
                              <Button
                                ...
                                size="sm"
                                variant="default"
                                title={contrato.SYNC_ATIVO ? "Sincronização automática está ativa" : "Sincronização completa"}
                              >
                                {isSyncing ? (
                                  <>
                                    <RefreshCw className="w-4 h-4 mr-2 animate-spin" />
        ...
        <Card>
          <CardHeader>
            <CardTitle>Parceiros Sincronizados (Detalhes)</CardTitle>
            <CardDescription>
              Lista de parceiros sincronizados do Sankhya (últimos 500).
            </CardDescription>
          </CardHeader>
          <CardContent>
                  ...
                  {parceiros.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={5} className="text-center py-8 text-muted-foreground">
                        Nenhum parceiro sincronizado encontrado.
                      </TableCell>
                    </TableRow>
                  ) : (
                    parceiros.map((parceiro) => (
                      <TableRow key={`${parceiro.ID_SISTEMA}-${parceiro.CODPARC}`}>
                        <TableCell>{parceiro.CODPARC}</TableCell>
                        <TableCell>{parceiro.NOMEPARC}</TableCell>
                        <TableCell className="text-xs text-muted-foreground">
                          {parceiro.NOME_CONTRATO} (ID: {parceiro.ID_SISTEMA})
                        </TableCell>
                        <TableCell>
                          {parceiro.SANKHYA_ATUAL === 'S' ? (
                        ...
                        <TableCell className="text-xs text-muted-foreground">
                          {formatarData(parceiro.DT_ULT_CARGA)}
                        </TableCell>
                      </TableRow>
                    ))
                  )}
